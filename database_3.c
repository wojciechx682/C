...
{	
	char fileName[100];
	sprintf_s(fileName, "pliczek.txt");
	FILE * outputFiles;
	fopen_s(&outputFiles, fileName, "w");
		
	int x = nr_pliku;
	fprintf_s(outputFiles, "%d\n", x);
...
void nowy()
{
	...
	char fileName[100];
	FILE * outputFile;
	sprintf_s(fileName, "rekord %d.txt", nr_pliku);
...
void aktualizacja()
{
	printf("Podaj numer rekordu do zaktualizowania: ");
	scanf_s("%d", &nr_pliku);
